  ...
  bcrypt.hash(req.body.password, saltRounds, (error, hash) => {
    if (!error) {
      pool.getConnection((err, db) => {
        const query = 'INSERT INTO users (user_role, email, password, first_name, last_name, class_id) VALUES (?, ?, ?, ?, ?, ?)';
    ...
    db.query(query, [req.params.studentId], async (error, result, fields) => {
      if (result && result.length) {
        ...
        for (const r of result) {
          // create new object
          const entry = {
            firstName: r.first_name, lastName: r.last_name, classStartDate: r.start_date_time, teacher_name: r.teacher_id, teacher_surname: r.teacher_id, className: r.name, courseName: r.courseName, isAttending: r.is_attending,
          };
...
router.get('/api/users/lectures/:teacherId', (req, res) => {
  pool.getConnection((err, db) => {
    ...
    db.query(query, [req.params.teacherId, `${yyyy}-${mm}-${dd}`], async (error, result, fields) => {
      console.log(result);
      if (result && result.length) {
        const todayClasses = result.map((c) => ({ lecture_id: c.lecture_id, name: c.name, start_date_time: c.start_date_time }));
    ...
    db.query(query, [req.params.teacherId, req.body.data.class_id, req.body.data.course_id], async (error, result, fields) => {
      if (result && result.length) {
        ...
        for (const r of result) {
          // create new object
          attendance.push({
            firstName: r.first_name, lastName: r.last_name, email: r.email, classStartDate: r.start_date_time, isAttending: r.is_attending,
          });
        }
        date = new Date();
