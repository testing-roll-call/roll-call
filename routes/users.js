...
router.get('/api/users/students/attendance/:studentId', (req, res) => {
    pool.getConnection((err, db) => {
        let query = 'SELECT users.first_name, users.last_name, lectures.start_date_time, lectures.teacher_id, classes.name, attendance.is_attending, courses.name AS courseName from users join attendance on users.user_id = attendance.user_id join lectures on attendance.lecture_id = lectures.lecture_id join courses on courses.course_id = lectures.course_id join classes on classes.class_id = lectures.class_id where users.user_id = ?;';
...
router.get('/api/users/classes/courses/all/:teacherId', (req, res) => {
    pool.getConnection((err, db) => {
        let query = 'SELECT DISTINCT courses.name AS courseName, classes.name AS className, courses.course_id, classes.class_id from courses join lectures on courses.course_id = lectures.course_id join classes on classes.class_id = lectures.class_id where lectures.teacher_id = ?;';
    ...
    pool.getConnection((err, db) => {
        let query = `SELECT users.first_name, users.last_name, users.email, lectures.start_date_time, attendance.is_attending
                    ...
                    WHERE lectures.teacher_id = ? AND 
                        ...
                        lectures.course_id = ?;`;
        db.query(query, [req.params.teacherId, req.body.data.class_id, req.body.data.course_id], async (error, result, fields) => {
            if (result && result.length) {
                const attendance = [];
                for (const r of result) {
                    //create new object
